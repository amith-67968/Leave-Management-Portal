  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><h1 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight">Manage Users</h1><p className="text-muted-foreground mt-1">Add and manage organization users</p></div>
        <button onClick={() => setShowForm(!showForm)} className="w-full px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-semibold hover:bg-primary/90 transition-colors sm:w-auto">{showForm ? 'Close Form' : '+ Add User'}</button>
      </div>
      ...
      <form onSubmit={handleHolidayCredit} className="bg-card border border-border rounded-xl p-6 max-w-4xl space-y-4">
        ...
        <button type="submit" className="w-full px-5 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-semibold hover:bg-primary/90 transition-colors sm:w-auto">Credit Comp-Off</button>
      </form>

      {loading ? <p className="text-muted-foreground">Loading...</p> : (
        <div className="bg-card border border-border rounded-xl overflow-hidden"><div className="overflow-x-auto"><table className="min-w-[720px] w-full text-sm">
